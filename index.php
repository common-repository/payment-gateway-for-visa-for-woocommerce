...
if(!function_exists("woocommerce_wc_visa_init")){
	function woocommerce_wc_visa_init()
	{
		...
		class WC_Gateway_VISA extends WC_Payment_Gateway
		{
            ...
            public $alnazer_transient_key = "alnazer_visa_transient_key";
				...
				$this->account_details = get_option(
					...
					array(
						array(
							...
							'visa_country' => $this->get_option('visa_country') ?? [],
            ...
            protected function alnazer_init_payments($payment_method){
                
                if($payment_method){
                    if(!is_numeric($payment_method)){
                        $payment_method = (int) filter_var($payment_method, FILTER_SANITIZE_NUMBER_INT);
                    }
                    
                    $all_options = get_transient($this->alnazer_transient_key);
                    if(isset($all_options) && array_key_exists($payment_method,$all_options)){
                        $init_data = $all_options[$payment_method];
                        if(is_array($init_data)){
                            foreach ($init_data as $key => $init_datum) {
                                $this->{$key} = $init_datum;
                            }
                        }
                    }
			...
			/**
			 * set Api url
			 * like error.cancel.response ... ect
			 */
			 ...
			 *
			 * @param string $selections Field key.
			 * @since  1.0.0
			 * @return string
			 */

			public function generate_multiselect_html($field_name ='countries',$selections = []) {
				...
				if (!empty($countries)) {
					foreach ($countries as $key => $val) {
						$selection_values = (is_array($selections) && count($selections) > 0)? array_values($selections) : array();
						$list.='<option value="' . esc_attr($key) . '"' . wc_selected($key, $selection_values) . '>' . esc_html($val) . '</option>';
					}
				}
				asort( $countries );
				return '<select multiple="multiple" name="'.esc_attr( $data['field_name'] ).'[]" data-placeholder="'.__( 'Choose countries or regions&hellip;', 'woocommerce' ).'" aria-label="'.__( 'Country or Region', 'woocommerce' ).'" class="wc-enhanced-select">'.$list.'</select>';
			}
			/**
						...
						<div class="wc_input_table_wrapper">
							<table class="widefat wc_input_table sortable" cellspacing="0">
								...
								<tr>
                                    ...
                                    <th><?php esc_html_e('Gateway Url', 'wc_visa'); ?></th>
									<th><?php esc_html_e('type', 'wc_visa'); ?></th>
									<th><?php esc_html_e('Interaction', 'wc_visa'); ?></th>
									<th><?php esc_html_e('commission', 'wc_visa'); ?> %</th>
									<th><?php esc_html_e('Allow in countries', 'wc_visa'); ?></th>
								...
								if ($this->account_details) {
									foreach ($this->account_details as $account) {
										...
										echo '<tr class="account">
											...
											<td><input type="text" value="' . esc_attr($account['gateway_url'] ?? "") . '" name="gateway_url[' . esc_attr($i) . ']" /></td>
											...
											<td>'.$this->generate_multiselect_html("visa_country[" . esc_attr($i) . "]",$account['visa_country'] ?? []).'</td>
											<td><input type="text" value="' . esc_attr($account['visa_merchantid']) . '" name="visa_merchantid[' . esc_attr($i) . ']" /></td>
											<td><input style="width: 150px !important;" type="password" value="' . esc_attr($account['visa_password']) . '" name="visa_password[' . esc_attr($i) . ']" /></td>
										</tr>';
									}
								}
								...
								$select_country = $this->generate_multiselect_html("visa_country[' + size + ']",[]);
								?>
								</tbody>
								<tfoot>
								<tr>
									<th colspan="7">
										<a href="#" class="add button"><?php esc_html_e('+ Add account', 'wc_visa'); ?></a>
										<a href="#" class="remove_rows button"><?php esc_html_e('Remove selected account(s)', 'wc_visa'); ?></a>
									</th>
								</tr>
								</tfoot>
							</table>
							...
							jQuery(function () {
								jQuery('#<?php  echo $this->visa_account_slug; ?>').on('click', 'a.add', function () {
									...
									jQuery('<tr class="account">\
										...
										<td  style="text-align:center" ><input style="text-align:center" type="checkbox" value="true" checked  name="visa_active[' + size + ']" /></td>\
										<td  style="text-align:center" ><?php echo $select_is_test; ?></td>\
										<td><input  type="text" name="gateway_url[' + size + ']" /></td>\
										<td><?php echo $select; ?></td>\
										<td><?php echo $select_interaction ?></td>\
										<td><input type="number" step="0.1" min="0" name="visa_commission[' + size + ']" /></td>\
										<td><?php echo $select_country ?></td>\
										<td><input type="text" name="visa_merchantid[' + size + ']" /></td>\
										<td><input style="width: 150px !important;" type="password" name="visa_password[' + size + ']" /></td>\
			...
			public function save_account_details()
			{
				...
				if (isset($_POST['visa_title']) && isset($_POST['visa_type']) && isset($_POST['visa_interaction'])
					...
					$visa_countries = wc_clean(wp_unslash($_POST['visa_country'] ?? []));
                    ...
                    $gateway_urls = wc_clean(wp_unslash($_POST['gateway_url'] ?? ""));
			...
			{
                return get_option( 'order_has_id_'.$order->get_id());
			}
			...
			protected function visa_request($operation = "INITIATE_CHECKOUT", $body = [])
			{
				...
				$parserObj->FormRequestUrl($merchantObj);
			...
			function process_payment($order_id)
			{
				...
				$body = array(
                    ...
                    "interaction.returnUrl" => get_site_url().'?wc-api='.$this->responceUrl.'&order_id='.$order_id.'&order_hash='.$visa_order_id,
                    "interaction.cancelUrl" => get_site_url().'?wc-api='.$this->cancelUrl.'&order_id='.$order_id.'&order_hash='.$visa_order_id,
                    'interaction.timeoutUrl' => get_site_url().'?wc-api='.$this->errorUrl.'&order_id='.$order_id.'&order_hash='.$visa_order_id,
			...
			public function form()
			{
				...
				if ($order_id && $order) {
                    ...
                    $this->alnazer_init_payments($order->get_payment_method());
                    
                    $visa_order_id = $this->getOrderHash($order);
					...
					$replace = [
                        ...
                        "{{session_version}}" => $this->getSession('session.version'),
						"{{checkoutUrl}}" => $this->checkoutUrl,
						'{{language}}' => $this->language,
						'{{cancel_link}}' => $this->cancelUrl,
						'{{error_link}}' => $this->errorUrl,
						'{{response_link}}' => $this->responceUrl,
					];
     
					echo $template = str_replace(array_keys($replace), array_values($replace), $template);
					exit();
				} else {
			...
			public function visa_responce()
			{
				...
				if ($order) {
                    $this->alnazer_init_payments($order->get_payment_method());
					$orderID =  $this->getOrderHash($order);
                    ...
                    if (isset($parsed_array['error.explanation']) && !empty($parsed_array['error.explanation'])) {
                        $_note .= sprintf("VISA ERROR : %s <br/>", $parsed_array['error.explanation']);
                    }
            ...
            private function setOrderHash(WC_Order $order, string $visa_order_id)
            {
                return update_option( 'order_has_id_'.$order->get_id(),$visa_order_id);
            }
            
        }

		/**
		 * to maje multi account
		 */
		class WC_Alnazer_Gateway_VISA_NEW extends WC_Gateway_VISA
		{
			...
			{
				parent::__construct();
				$this->init($config,$key);
				// set new default attr
				$this->setConfigArray();
			}
			 ...
			 * @return void
			 */
			protected function init($config = [],$_key)
			{
				$this->icon = plugins_url('images/visa-logo.png', __FILE__);
				$this->method_description = __('Easy and fast and secure', 'wc_visa');
				$this->has_fields = false;
				if ($config && count($config) > 0) {
                    ...
                    $get_transient = get_transient($this->alnazer_transient_key);
                    $get_transient[$_key] = $config;
                    set_transient($this->alnazer_transient_key,$get_transient,DAY_IN_SECONDS );
				}
			}
		}
			...
			if (is_admin() && $page === "wc-settings" && $tab === "checkout") {
				...
				if (isset($alnazer_visa_account_list) && is_array($alnazer_visa_account_list) && count($alnazer_visa_account_list) > 0) {
					foreach ($alnazer_visa_account_list as $key => $item) {
						...
						$alnazer_get_user_country = alnazer_get_user_country();
						if(!empty($alnazer_get_user_country ) && isset($item["visa_country"]) && is_array($item["visa_country"]) && count($item["visa_country"]) > 0 && !in_array($alnazer_get_user_country,$item["visa_country"])){
							continue;
						}
						$config = [
							"id" => "wc_visa_$key",
							...
							"country" => $item["visa_country"] ?? [],
							"apiusername" => "merchant." . $item["visa_merchantid"],
							"password" => $item["visa_password"],
							'is_test' => $item['visa_is_test'],
						];
                        if(!empty($item["gateway_url"])){
                            $config['gatewayUrl'] = alnazer_format_url($item["gateway_url"]);
                        }
						$methods[] = new WC_Alnazer_Gateway_VISA_NEW($config,$key);
					}
				}
        ...
        if(!function_exists("alnazer_format_url")){
            function alnazer_format_url($url){
                $url = trim($url);
                $url = str_replace(["","https://","https://","www.",'api/nvp'],"",$url);
                return "https://".$url."/api/nvp";
            }
        }
		if(!function_exists("alnazer_get_user_country")){
			function alnazer_get_user_country(){
				try {
					$list = unserialize(file_get_contents('http://www.geoplugin.net/php.gp?ip='.alnazer_get_user_ip()));
					return (isset($list['geoplugin_countryCode']) && !empty($list['geoplugin_countryCode'])) ? $list['geoplugin_countryCode'] : null;
				}catch (\Exception $e){
				return  null;
				}
			}
		...
		if(!function_exists("alnazer_get_user_ip")){
			function alnazer_get_user_ip(){
